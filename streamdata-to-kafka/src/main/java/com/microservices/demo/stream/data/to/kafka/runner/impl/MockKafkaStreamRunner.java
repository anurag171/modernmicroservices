...
import com.microservices.demo.stream.data.to.kafka.exception.StreamDataToKafkaServiceException;
...
public class MockKafkaStreamRunner implements StreamRunner {
    ...
    public void start() throws TwitterException {
        final String[] keywords = streamDataToKafkaConfig.getTwitterKeywords().toArray(new String[0]);
        final int minTweetLength = streamDataToKafkaConfig.getMockMinTweetLength();
        final int maxTweetLength = streamDataToKafkaConfig.getMockMaxTweetLength();
        long sleepTimeMs = streamDataToKafkaConfig.getMockSleepMs();
        ...
        Executors.newSingleThreadExecutor().submit(() -> {
            try {
                while (true) {
                    ...
                    streamDataStatusListener.onStatus(status);
                    sleep(sleepTimeMs);
                }
    ...
    private void sleep(long sleepTimeMs) {
        try {
            ...
            throw new StreamDataToKafkaServiceException("Error while sleeping for waiting new status to create!!");
        }
    }
